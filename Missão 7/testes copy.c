...
Node* bringToTop(Node* head, int data) {
    Node* current = head ;
    Node* topp = malloc(sizeof(Node)) ;

    while (current) {
        Node* next = (Node*) malloc(sizeof(Node));
        int nextNodeIndex = nodeIndex(head, current -> data) + 1 ;

        if (nextNodeIndex == -1)
            free(next);
        else
            next = getNodeByIndex(head, nextNodeIndex);

        Node* previous = (Node*) malloc(sizeof(Node));
        int previousIndex = nodeIndex(head, current -> data) - 1 ;
...
int main () {
    ...
    printf("%s\n", separator);
    printf("List with index #2 moved to the top:\n%s\n",separator);

    head = bringToTop(head, getNodeByIndex(head,2));
    showChain(head);
